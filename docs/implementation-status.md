...
| Marble live-control performance | In progress | engineering-preview | P0-P3 are complete; P4 safe transitions and the first P5 high-rate coordinator slice are implemented. P2 reduces extreme 10/80/10 from a 1,938 ms median to 105.8 ms p95. P3's pooled scene passed 100 updates at 1.8 ms application p95 and 17.3 ms frame p95 with 11 geometries, 7 programs, renderer identity `1`, and one overlay. P4 aligns incoming route/targets at a shared impact, blends the camera, and morphs future same-shape targets only after 12 sampled layouts pass SAT and active-marble clearance. P5 replaces release debounce with a 100 ms fixed request cadence plus trailing request, retains valid in-flight work, accepts progressive latest results, starts morphs at current transport time, and retargets from displayed transforms. A browser drag of 18 steps at 85 ms produced 16 distinct validated plans before release, progressing from 20/25/55 to 20/41/39 with 16.8 ms frame p95. | Finish P5 diagnostics, deadband/filtering, and sustained-stream gates in [Marble live-control performance](implementation/marble-live-control-performance.md), alongside the remaining P4 played-target and natural audio watch-through gates, before MediaPipe |
