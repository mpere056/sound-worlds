...
| Aurora Cyclotron A0-A1 | In progress | headless physics foundation | `@reaper-viz/compiler-aurora`, `compile:aurora`, deterministic source selection, chord grouping, stable deadline IDs, option validation, CLI artifact output, and the constant-field Lorentz kernel are implemented. The kernel supports arbitrary constant E/B fields, circular and helical motion, E x B drift, neutral/zero-field degeneracies, and work-energy diagnostics, with a 50,000-step Boris comparison. The reference song compiles 19 notes into 19 deadlines from 0.531250 to 9.916146 seconds with a 0.174479-second minimum gap. No route, coil geometry, scene, or shader is advertised yet. | Finish A1 divergence-free finite-field/fringe primitives, then begin A2 inverse field and coil solving; remain at Q0 until exact coil-center routes certify |
