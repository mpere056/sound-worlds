...
| Aurora Cyclotron A0-A2 | A0-A1 complete; A2 in progress | headless physics and inverse-segment foundation | `@reaper-viz/compiler-aurora`, `compile:aurora`, deterministic source selection, chord grouping, stable deadline IDs, option validation, CLI artifact output, and the constant-field Lorentz kernel are implemented. The kernel supports arbitrary constant E/B fields, circular and helical motion, E x B drift, neutral/zero-field degeneracies, and work-energy diagnostics. A divergence-free paraxial finite-solenoid model, aperture validity metric, numerical divergence probe, and variable-field Boris integrator cover smooth fringes. The first A2 primitive now inversely solves bounded signed magnetic fields for authored turn angles and emits exact helical coil-center contacts with physical coil axes kept distinct from arrival directions. Twenty-three focused Aurora tests pass. The reference song compiles 19 notes into 19 deadlines from 0.531250 to 9.916146 seconds with a 0.174479-second minimum gap. No global route, coil geometry, scene, or shader is advertised yet. | Add deterministic planar/3D/long-gap candidate families and route costs, then apply bounded finite-fringe shooting correction; remain at Q0 until full exact coil-center routes certify |
