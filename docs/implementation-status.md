...
| Marble live-control performance | In progress | engineering-preview | P0-P3 are complete; P4 safe transitions and the first P5 high-rate coordinator slice are implemented. P2 reduces extreme 10/80/10 from a 1,938 ms median to 105.8 ms p95. P3 retains one pooled renderer/scene. P4 aligns at a shared impact and blends the camera. Collision-safe targets morph transforms; withheld, shape-changing, and played targets now use independent opacity crossfades at stable size, inheriting displayed opacity across repeated plans. The contact platform stays solid through impact and fades afterward. The final-note rebound bug is fixed, removing the 15/26/59 emergency speck. Tiny collision pads required by dense extremes now sit on bounded visible carriers: compact 0.22 x 0.055 x 0.14 to 0.5 x 0.18 x 0.36, full 0.48 x 0.09 x 0.24 to 1.35 x 0.28 x 0.7; rendered bases are maximum-capped. A global-spacing alternative was rejected for exceeding the 3.3 speed ceiling. P5 submits every 100 ms and retargets displayed state. The exact 15/26/59 browser check retained full platform dimensions through 1.620 s and opacity-transitioned across the 1.916 s activation with 16.8 ms frame p95. | Finish P5 diagnostics, deadband/filtering, sustained-stream gates, and a natural audio transition watch-through before MediaPipe |
