...
| Marble live-control performance | In progress | engineering-preview | P0 instrumentation is implemented. `benchmark:marble-live` reports compiler phases and deterministic candidate/clearance counters; `?profileMarble=1` records frame, long-task, scene-swap, first-render, and Three.js resource diagnostics without visible production UI. The 10/80/10 reference profile measures 1.94 s median in warm Node and 5.30 s in the dev browser; target placement consumes 5.289 s with 14,451 candidates and 8.5 million clearance samples, while scene replacement is 9.3 ms and first render is 115.6 ms. The dedicated plan continues with latest-only worker planning, targeted compiler optimization, persistent renderer/resource pools, collision-aware morphing, high-rate input coordination, worker-based MediaPipe, and a measured WASM decision gate | Begin P1 in [Marble live-control performance](implementation/marble-live-control-performance.md): move planning off the UI thread with project/request generations and latest-pending coalescing, then use P0 counters to drive P2 target-placement optimization |
