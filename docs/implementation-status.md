...
| Marble live-control performance | In progress | engineering-preview | P0-P2 are implemented. `benchmark:marble-live` reports compiler phases and deterministic work counters; `?profileMarble=1` records frame, long-task, worker compile, scene-swap, first-render, and Three.js resources. A module planner worker retains the active song, uses project generations and request IDs, coalesces pending work, reports optional profiles, and leaves the active valid world usable while compiling. The client rejects stale results, invalidates on raw input, checks planned versus desired mix, and delays activation through a short quiet period. A 100-request unit burst activates only the newest result. P2 adds a deterministic 3D route spatial hash, one-time orientation filtering, early clearance rejection, and adaptive contained-scale preflights. Across 12 warm reference runs, default 20/20/60 is 33.2 ms p95 and extreme 10/80/10 is 105.8 ms p95, down from a 1,938 ms median. The extreme now evaluates 3,207 candidates and 142,961 clearance samples instead of 14,451 and 8.5 million. | Begin P3 in [Marble live-control performance](implementation/marble-live-control-performance.md): preserve one renderer and scene, then pool target and route resources before P4 collision-aware morphing |
