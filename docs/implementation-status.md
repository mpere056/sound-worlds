...
| Marble live-control performance | In progress | engineering-preview | P0-P2 are complete and P3 renderer ownership is implemented. `benchmark:marble-live` reports compiler phases and deterministic work counters; `?profileMarble=1` records frame, long-task, worker compile, scene-application, first-render, stable renderer identity, update count, and Three.js resources. The latest-only planner worker leaves the active valid world usable while compiling and rejects stale project/request generations. P2's spatial hash, orientation filtering, early rejection, and adaptive scale preflights reduce the extreme 10/80/10 reference from a 1,938 ms median to 105.8 ms p95. `MarbleScene.replacePerformance()` now retains its renderer, context, scene, camera, lights, wall, marble, tuning, and single SVG overlay while replacing performance-owned geometry. Two browser updates retained renderer identity `1`; scene application was 8.8-11.7 ms and first render was 20.3-24.7 ms versus the prior 119 ms. | Continue P3 in [Marble live-control performance](implementation/marble-live-control-performance.md): pool target and route geometry/materials to reach <=4 ms application and pass the 100-change bounded-resource gate before P4 morphing |
