...
| Marble live-control performance | In progress | engineering-preview | P0-P3 are complete. The latest-only worker keeps playback responsive and rejects stale generations. P2's deterministic spatial hash, orientation filtering, early rejection, and adaptive scale preflights reduce extreme 10/80/10 from a 1,938 ms median to 105.8 ms p95. P3's `MarbleScene.replacePerformance()` retains the renderer, context, scene, camera, lights, wall, marble, tuning, and one SVG overlay; unit primitives and scene-lifetime materials are shared across target bases, hardware, rods, ties, and supports. In a fresh 100-update enhanced-profiler run, scene-application p95 was 1.8 ms, first-render p95 13 ms, render p95 12.7 ms, and frame p95 17.3 ms. Geometry stayed exactly 11, programs 7, renderer identity `1`, overlay count 1, and update count reached 100. | Begin P4 in [Marble live-control performance](implementation/marble-live-control-performance.md): activate validated routes at a safe future impact and morph future targets without marble, camera, or platform teleportation |
