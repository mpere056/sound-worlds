...
| Marble Music M0-M1.5 | In progress | engineering-preview | `@reaper-viz/compiler-marble`, `@reaper-viz/scene-marble`, `compile:marble`, Three.js app routing, one-track auto-selection, compiled impacts/path/tail, camera keys, and compiler tests are implemented. Compiler v11 uses a normalized lateral/vertical/depth motion profile to derive launch speed and gravity while keeping pitch out of route geometry and separating `target.contactPos` from rendered platform `target.pos`. The preview loads the compiled 20/20/60 default, exposes three linked 10-80% sliders, proportionally compensates the other axes to keep a 100% total, and deterministically recompiles after a 100 ms debounce without changing playback time or visual tuning. Route planning follows a five-forward/one-back depth cadence; platforms receive real depth tilt from 3D collision normals. Marble clearance is sphere-to-oriented-box distance, platform overlap uses the full 3D separating-axis set, and target candidates are ranked against a 120 Hz route sample. The reference project retains 19 exact impacts; its default measures 19.8% lateral / 20.4% vertical / 59.9% depth, with 15 forward and 3 backward transitions. Tests cover default plus lateral-, vertical-, and depth-heavy profiles. Browser checks confirm normalized live rebuilding at a scrubbed timestamp, one active SVG/WebGL scene, visible marble framing, and no console warnings. Full audio watch-through and full-timeline viewport gates remain open; move recompilation to a worker before supporting substantially larger live-edited songs. | Continue [Marble Music 3D physics-feel](implementation/marble-music-3d-physics-implementation.md): add full-timeline viewport projection coverage, run a full audio watch-through with exact-impact scrubs, then improve dense mechanism identity before cinematic camera offsets or two-track work |
