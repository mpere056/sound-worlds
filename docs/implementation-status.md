...
| Music-Synced Brick Breaker | In progress | selectable B1 preview | B0 deterministically selects one note-bearing track, groups chords, emits stable destruction deadlines, preserves the exact final note, and reports brick/cell counts plus gap diagnostics. The reference song requires 19 bricks with a 0.174479-second minimum gap. B1 compiles a constant-speed, absolute-time arcade trajectory with deterministic brick-face reflections, side/top wall bounces, bottom paddle returns, trajectory-authored brick poses, and a moving compiled paddle. The ball center is offset from each brick face by its radius, outgoing headings preserve horizontal momentum while reversing vertical travel, and support contacts emit visible rhythmic events. `Brick Breaker · B1 Preview` renders the route, fragments, trail, paddle, and exact note-time removals. | Add B2 continuous live-brick occupancy search so future bricks cannot overlap or be struck early, then certify the full route |
