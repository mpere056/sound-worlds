...
| Aurora Cyclotron A0-A1 | Implemented | headless physics foundation | `@reaper-viz/compiler-aurora`, `compile:aurora`, deterministic source selection, chord grouping, stable deadline IDs, option validation, CLI artifact output, and the constant-field Lorentz kernel are implemented. The kernel supports arbitrary constant E/B fields, circular and helical motion, E x B drift, neutral/zero-field degeneracies, and work-energy diagnostics. A divergence-free paraxial finite-solenoid model, aperture validity metric, numerical divergence probe, and variable-field Boris integrator now cover smooth fringes. Eighteen focused tests include a 50,000-step constant-field comparison and finite-fringe convergence and energy checks. The reference song compiles 19 notes into 19 deadlines from 0.531250 to 9.916146 seconds with a 0.174479-second minimum gap. No route, coil geometry, scene, or shader is advertised yet. | Begin A2 inverse ideal-field segment and coil solving, then apply bounded finite-fringe shooting correction; remain at Q0 until exact coil-center routes certify |
