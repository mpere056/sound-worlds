...
| Marble Music M0-M1.5 | In progress | engineering-preview | `@reaper-viz/compiler-marble`, `@reaper-viz/scene-marble`, `compile:marble`, Three.js app routing, one-track auto-selection, compiled impacts/path/tail, camera keys, and compiler tests are implemented. Compiler v8 uses controlled horizontal/depth launch velocity and gravity to place note-time collisions, keeps pitch out of route geometry, and separates `target.contactPos` from rendered platform `target.pos`. Contact points weave through a bounded front/back volume and platforms receive real depth tilt from three-dimensional collision normals. Marble clearance is sphere-to-oriented-box distance, while platform overlap uses the full 3D separating-axis set. Target candidates are ranked against a 120 Hz sample of the full route, so avoidable between-impact clipping is rejected or replaced by a shorter/rotated target. `untitled-project-418cb58f` retains 19 exact impacts, spans depth 0.25-3.25, keeps average segment speed bounded at 1.73-2.94, and has zero physical target intersections with positive full-route clearance. The camera now follows lateral/vertical motion from an elevated stable depth anchor rather than following marble depth; near/far impacts span 0.27 normalized screen height and change apparent marble scale by 29%. Unit and browser checks show continuous framing, visibly receding platform corridors, and no console warnings, but full audio watch-through and full-timeline viewport gates remain open. | Continue [Marble Music 3D physics-feel](implementation/marble-music-3d-physics-implementation.md): add full-timeline viewport projection coverage, run a full audio watch-through with exact-impact scrubs, then improve dense mechanism identity before cinematic camera offsets or two-track work |
