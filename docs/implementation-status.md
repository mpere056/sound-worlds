...
| Marble Music M0-M1.5 | In progress | engineering-preview | `@reaper-viz/compiler-marble`, `@reaper-viz/scene-marble`, `compile:marble`, Three.js app routing, one-track auto-selection, compiled impacts/path/tail, camera keys, and compiler tests are implemented. Compiler v5 uses controlled launch speed and gravity to place note-time collisions, keeps pitch out of route geometry, and now separates `target.contactPos` from rendered platform `target.pos`. Platforms sit behind contact by marble radius plus target half-thickness and clearance; tests sample before/at/after each impact to reject penetration. Launch direction prefers unoccupied route space, platform angles try physically valid alternatives, target sizes shrink or become compact pegs/chimes when crowded, and inflated oriented footprints are compile-time validated. `untitled-project-418cb58f` retains 19 exact impacts and bounded 1.64-2.76 average speed with zero target-footprint intersections. Compact hardware mounts and target blooms are reduced to keep dense mechanisms readable. The pose-follow camera keeps the marble visible near center. Browser review shows a clean initial frame and no console warnings, but full audio watch-through and full-timeline viewport gates remain open. | Continue [Marble Music 3D physics-feel](implementation/marble-music-3d-physics-implementation.md): add full-timeline viewport projection coverage, run a full audio watch-through with exact-impact scrubs, then improve dense mechanism identity before cinematic camera offsets or two-track work |
