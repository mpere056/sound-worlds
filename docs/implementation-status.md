...
| Marble live-control performance | In progress | engineering-preview | P0-P3 are complete and P4's activation-alignment kernel is implemented. The latest-only worker keeps playback responsive and rejects stale generations. P2 reduces extreme 10/80/10 from a 1,938 ms median to 105.8 ms p95. P3 retains one renderer/context/scene and pools primitives/materials; its 100-update run measured 1.8 ms scene-application p95, 13 ms first-render p95, 12.7 ms render p95, and 17.3 ms frame p95 with exactly 11 geometries, 7 programs, renderer identity `1`, and one overlay. P4 now selects the next shared note impact and rigidly aligns every incoming path/target/control/camera position so old and new marble centers coincide while preserving all relative collision geometry and exact timestamps. Three tests cover continuity, clearance, spacing, immutability, and no-boundary behavior. | Wire the P4 boundary into live playback, blend camera state, then add collision-checked future-target morphing in [Marble live-control performance](implementation/marble-live-control-performance.md) |
