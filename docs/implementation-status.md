...
| Lumenfall L0-L8 | Concept and work orders defined | planned | The [concept](lumenfall.md) defines a luminous body traversing a permanent wet-stone night world, with one exact surface contact per note, analytic gravity arcs, bounded contact impulses, and position-driven physically based illumination. The [implementation plan](implementation/lumenfall-implementation.md) specifies static world/collision contracts, pair-state route search, continuous collision certification, PBR materials, inverse-square lighting, shadow and occlusion gates, contact bounce-light energy limits, cinematic camera constraints, and a five-impact vertical slice. No package or artifact exists yet. | Implement L0-L1 fixtures and Nocturne Causeway graybox, then prove one certified five-impact route and realistic lighting with bloom disabled before full-song or material polish work |
