...
| Physics-first visual worlds | Three engineering previews; two strengthened plans | in progress | Aurora Cyclotron, Phaseglass, and Vortex Loom now have implemented one-track engineering previews. Pendulum Cathedral and Singularity Slalom retain implementation-ready plans with deliberately different foundations: readable nonlinear mechanics and compiled orbital geometry with physically owned lensing. All five share exact event ownership, deterministic offline compilation, anticipation, seek-safe state, certification artifacts, and the architecture-neutral Q0-Q5 [visual-quality standard](implementation/shader-worlds-visual-quality-standard.md). | Continue each world's visual acceptance independently. Do not reuse Vortex Loom's transported-fiber grammar as a generic shader-world template |
| Vortex Loom V0-V6 | Initial certified structural preview implemented | engineering-preview | `@reaper-viz/compiler-vortex-loom` groups one note-bearing track, compiles a chamber-confined divergence-free stream-function field, solves an absolute-time RK4/Hermite shuttle route, and emits deterministic vortex, interaction, fiber, checkpoint, and certification artifacts. An independent pass verifies exact first annulus entries, inward radial speed, no early ownership, core clearance, chamber containment, and numerical divergence. After visual review rejected the raw checkpoint polyline forest, `@reaper-viz/scene-vortex-loom` now resamples the real transported state into subdued spline threads inside one signed-circulation silk field. A short material stitch establishes recent motion; the next note opens and colors an aperture between the shuttle and its destination; contact light inherits the struck note's pitch pigment; velocity controls concentration; and encountered pigment persists with deterministic relaxation rather than disappearing after the flash. The 19-deadline reference has zero entry error, zero early entries, `2.11e-6` maximum numerical divergence, `0.0042` minimum core clearance, 51 deterministic checkpoints, and zero violations. | Replace the sequential fixed-point placement with the planned independent root oracle plus deterministic beam/multiple-shooting global solve (V2-V4); then add true remeshing and advected pigment checkpoint payloads, worker replay, camera certification, stronger pitch/register spatial grammar, and Q1-Q5 acceptance (V5-V10) |
