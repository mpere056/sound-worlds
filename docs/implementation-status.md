...
| Vortex Loom V0-V6 | Initial certified structural preview implemented | engineering-preview | `@reaper-viz/compiler-vortex-loom` groups one note-bearing track, compiles a chamber-confined divergence-free stream-function field, solves an absolute-time RK4/Hermite shuttle route, and emits deterministic vortex, interaction, fiber, checkpoint, and certification artifacts. An independent pass verifies exact first annulus entries, inward radial speed, no early ownership, core clearance, chamber containment, and numerical divergence. `@reaper-viz/scene-vortex-loom` renders real checkpointed Lagrangian fiber positions, a shared transported pigment field, a directional three-second anticipation corridor, material-owned contact light, and a shader-woven shuttle knot. The 19-deadline reference has zero entry error, zero early entries, `2.11e-6` maximum numerical divergence, `0.0042` minimum core clearance, 51 deterministic checkpoints, and zero violations. App routing, controls, seeking, PNG/preview integration, compiler tests, scene tests, full typecheck, production build, and browser inspection pass. | Replace the sequential fixed-point placement with the planned independent root oracle plus deterministic beam/multiple-shooting global solve (V2-V4); then add remeshing/pigment checkpoint payloads, worker replay, camera certification, stronger musical-material grammar, unified absorption renderer, and Q1-Q5 acceptance (V5-V10) |
