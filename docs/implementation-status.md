...
| Marble live-control performance | In progress | engineering-preview | P0-P3 are complete; P4 safe transitions and the first P5 high-rate coordinator slice are implemented. P2 reduces extreme 10/80/10 from a 1,938 ms median to 105.8 ms p95. P3 retains one pooled renderer/scene. P4 aligns at a shared impact and blends the camera. Collision-safe targets morph transforms; unsafe targets remain visible on the active route, then outgoing visual ghosts crossfade against the validated incoming set for 350 ms after activation. The contact platform stays solid through impact and fades afterward. The final-note rebound bug is fixed, removing the 15/26/59 emergency speck. Tiny collision pads required by dense extremes now sit on colored marble-relative carriers: compact 0.58 x 0.11 x 0.28 to 0.82 x 0.2 x 0.46, full 0.68 x 0.12 x 0.32 to 1.35 x 0.28 x 0.7; collision bodies remain exact and rendered bases are maximum-capped. The reported 12/78/10 profile had 18 compact targets as short as 0.024 units; browser checks at 5.728 s and at the 43/47/10, 1.381 s frame now retain visible colored platform faces. A global-spacing alternative was rejected for exceeding the 3.3 speed ceiling. P5 submits every 100 ms and retargets displayed state. | Finish P5 diagnostics, deadband/filtering, sustained-stream gates, and a natural audio transition watch-through before MediaPipe |
