...
| Marble live-control performance | In progress | engineering-preview | P0-P3 are complete; P4 transform-only transitions and the first P5 high-rate coordinator slice are implemented. P2 reduces extreme 10/80/10 from a 1,938 ms median to roughly 100 ms p95. P3 retains one pooled renderer/scene. P4 now holds transport and marble time, aligns the incoming route at that exact timestamp, and moves one fully visible platform set for 450 ms before installing validated physics and resuming playback. Position, shortest-path rotation, collision-body size, colored carrier, hardware group, and support rods move continuously; no crossfade, reveal, ghost, duplicate target set, or transition opacity remains. Repeated worker plans retarget from displayed transforms, so continuous slider input does not restart from stale positions. Tiny collision pads required by dense extremes sit on colored marble-relative carriers: compact 0.58 x 0.11 x 0.28 to 0.82 x 0.2 x 0.46, full 0.68 x 0.12 x 0.32 to 1.35 x 0.28 x 0.7; collision bodies remain exact and rendered bases are maximum-capped. Scrub is briefly locked during re-layout, so backward playback always starts from a complete settled platform map. P5 submits every 100 ms and catches up to the final desired mix 450 ms after release. | Smooth decorative route-rail rebuilding, finish P5 diagnostics/deadband/filtering and sustained-stream gates, then run a natural audio transition watch-through before MediaPipe |
