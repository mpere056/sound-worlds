...
| Music-Synced Brick Breaker | In progress | B0 complete | The new headless compiler deterministically selects one note-bearing track, groups chords by a fixed anchor epsilon, emits stable destruction deadlines, preserves the exact final note, and reports required brick/cell counts plus gap diagnostics. It writes `brick-breaker.plan.json`, which is intentionally not exposed as a playable app world before trajectory certification. The reference song requires 19 bricks with a 0.174479-second minimum gap. | Implement B1 direct-contact vector/reflection primitives, absolute-time ball segments, and exact-deadline sampling |
