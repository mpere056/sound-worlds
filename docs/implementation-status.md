...
| Live Marble Music | In progress | foundation | This is a separate runtime with webcam gestures, REAPER MIDI telemetry, fixed-step physics, and a rolling 5-8-platform certainty window. The tested certainty state machine now provides stable IDs, explicit uncertain/solving/certain/spent ownership, generation-safe replanning, ordered impacts, and one-slot replenishment. Future platforms remain lightweight and non-collidable in an uncertainty vortex, settle continuously into certified poses, and lock before activation. | Add typed rolling-solver protocol and collision poses, then build the uncertainty renderer and REAPER telemetry/latency spike |
