...
| Marble live-control performance | In progress | engineering-preview | P0-P3 are complete; P4 transform-only transitions and the first P5 high-rate coordinator slices are implemented. P2 reduces extreme 10/80/10 from a 1,938 ms median to roughly 100 ms p95. P3 retains one pooled renderer/scene. P4 holds transport and marble time, aligns the incoming route at that timestamp, and moves one fully visible platform set over a displacement-aware 450-1400 ms window before installing validated physics and resuming playback. Position, shortest-path rotation, collision-body size, colored carrier, and stable-ID route rails move continuously. No crossfade, reveal, ghost, duplicate target set, or transition opacity remains. Repeated worker plans retarget from displayed state, so continuous slider input does not restart from stale positions. All collision kinds render one rectangular carrier-and-fastener assembly with no protruding compact cylinders. Dense or intersecting carrier components use one absolute-time representative: each target is visible at its own impact, including on random seek, while simultaneous overlapping carriers remain suppressed. Transition routing certifies those visible footprints. Fasteners remain attached to the carrier top; detached brackets, target rods, SVG target duplicates, rail ties, and support collars are removed. P5 owns pure bounded 100% projection plus gesture-ready deadband/slew filtering. P6 includes the explicit local MediaPipe worker. Index-thumb maps filtered palm x/y/scale into the bounded 3D motion mix; middle-thumb controls a separately filtered marble-locked camera orbit. | Tune spatial gain, depth response, pinch thresholds, low-light behavior, and sustained-session latency with the target webcam |
