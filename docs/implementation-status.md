...
| Marble Music M0-M1 | In progress | engineering-preview | `@reaper-viz/compiler-marble`, `@reaper-viz/scene-marble`, `compile:marble`, Three.js app routing, Pixi/Three backend ownership, one-track auto-selection, compiled note impacts/path/tail, authored camera keys, and compiler tests are implemented. `untitled-project-418cb58f` proves the first mostly-successful one-track sync behavior: one marble, exact note impacts, sustain-aware holds, and back-solved travel into the next onset. `untitled-project-6d2e04f7` compiles to 20 low-track impacts with zero dropped notes and audible-tail resonance | Implement [Marble Music 3D physics-feel](implementation/marble-music-3d-physics-implementation.md): physical pose sampler, real 3D machine geometry, distance-based spin, contact response, camera parallax, then dense high-track manual stress review and two-track duet planning |
