...
| Marble Music M0-M1.5 | In progress | engineering-preview | `@reaper-viz/compiler-marble`, `@reaper-viz/scene-marble`, `compile:marble`, Three.js app routing, Pixi/Three backend ownership, one-track auto-selection, compiled note impacts/path/tail, authored camera keys, and compiler tests are implemented. Compiler v3 replaces sustain-aware holds and late departures with continuous impact-to-impact motion: a delayed first note gets an accelerating drop, every impact launches the next full-interval segment immediately, free-flight arcs use constant-acceleration parabolas, rail motion retains endpoint velocity, spin accumulates across impacts, and the final note settles through the tail. `untitled-project-418cb58f` now compiles to 20 contiguous moving segments with zero holds; the former 94 ms late jump uses its full 735 ms note interval. M1.5 also includes 3D tangents/normals, arc-length samples, normalized quaternions, exact-impact and continuous-motion tests, paired Three.js tube rails with ties/standoffs/collars, unrailed airborne arcs, contact shadows, damped target recoil, wall-frame geometry, target hardware, pose-driven camera parallax, hidden SVG fallback, softer glow, and calmer framing. Browser checks show no console warnings, but the full human audio watch-through gate remains open. | Continue [Marble Music 3D physics-feel](implementation/marble-music-3d-physics-implementation.md): run a full audio watch-through, scrub exact impacts and several between-note positions, improve dense/local mechanism readability, do dense high-track manual stress review, and only then plan two-track duet work |
