...
| Marble live-control performance | In progress | engineering-preview | P0-P3 are complete; P4 safe transitions and the first P5 high-rate coordinator slice are implemented. P2 reduces extreme 10/80/10 from a 1,938 ms median to 105.8 ms p95. P3's pooled scene passed 100 updates at 1.8 ms application p95 and 17.3 ms frame p95 with 11 geometries, 7 programs, renderer identity `1`, and one overlay. P4 aligns route/targets at a shared impact and blends the camera. Collision-safe same-shape targets morph transforms; all withheld, shape-changing, and played targets now scale to 4% before activation and grow at their new transform, inheriting current scale across repeated plans. The contact platform remains solid through impact, then performs the same two-stage transition as the marble departs. P5 submits at a fixed 100 ms cadence and retargets from displayed state. An 18-step drag produced 16 validated intermediate plans with 16.8 ms frame p95; reference 20/40/40 transitions 18 of 19 platforms before impact, with the contact platform transitioning afterward, at 17.4 ms frame p95. | Finish P5 diagnostics, deadband/filtering, and sustained-stream gates in [Marble live-control performance](implementation/marble-live-control-performance.md), plus a natural audio transition watch-through, before MediaPipe |
