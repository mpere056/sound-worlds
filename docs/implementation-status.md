...
| Marble live-control performance | In progress | engineering-preview | P0-P3 are complete; P4 activation alignment, deferred playback, and camera blending are implemented. The latest-only worker keeps playback responsive and rejects stale generations. P2 reduces extreme 10/80/10 from a 1,938 ms median to 105.8 ms p95. P3 retains one pooled renderer/scene; its 100-update run measured 1.8 ms application p95 and 17.3 ms frame p95 with exactly 11 geometries, 7 programs, renderer identity `1`, and one overlay. P4 selects the next shared note impact, rigidly aligns the incoming route/targets to the active marble center, replaces older queued plans, and waits for transport to cross that impact. The first activation frame retains the old camera pose and smoothsteps to the incoming camera over 350 ms. A browser check queued 20/21/59 for 0.531 s with zero early swaps, then applied once after seeking across the boundary on the same renderer. Seven scene tests include continuity, clearance, spacing, immutability, no-boundary behavior, and camera blend endpoints. | Add collision-checked future-target morphing so platforms no longer adopt the aligned plan abruptly at activation, then complete the remaining P4 bridge/visual gates in [Marble live-control performance](implementation/marble-live-control-performance.md) |
