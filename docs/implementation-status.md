...
| Marble Music M0-M1.5 | In progress | engineering-preview | `@reaper-viz/compiler-marble`, `@reaper-viz/scene-marble`, `compile:marble`, Three.js app routing, Pixi/Three backend ownership, one-track auto-selection, compiled note impacts/path/tail, authored camera keys, and compiler tests are implemented. `untitled-project-418cb58f` proves the first mostly-successful one-track sync behavior: one marble, exact note impacts, sustain-aware holds, and back-solved travel into the next onset. M1.5 has begun with deterministic physical pose metadata: 3D tangents/normals, arc-length samples, distance-based spin, normalized quaternions, exact-impact pose tests, Three.js tube rails, contact shadows, damped target recoil, wall-frame geometry, target hardware, and pose-driven camera parallax. Browser still-frame review on `untitled-project-418cb58f` showed clear progress but did not pass the visual gate yet. `untitled-project-6d2e04f7` compiles to 20 low-track impacts with zero dropped notes and audible-tail resonance | Continue [Marble Music 3D physics-feel](implementation/marble-music-3d-physics-implementation.md): hide/demote the SVG fallback and plotted route line, clamp over-bright frame/glow response, keep camera parallax modest until rails/supports read physically, run a full audio watch-through, then do dense high-track manual stress review and two-track duet planning |
...
2. Review Marble Music M0-M1 in the browser, following
   ...
   pass shows directionally good progress. The next gate is stricter: with
   overlays off, remove/demote the graph-like fallback route language, prevent
   frame/glow over-brightness, and confirm through an audio watch-through that
   the low-track machine reads as synchronized and physical.
3. Use
