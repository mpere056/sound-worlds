...
| Marble live-control performance | In progress | engineering-preview | P0-P1 are implemented. `benchmark:marble-live` reports compiler phases and deterministic work counters; `?profileMarble=1` records frame, long-task, worker compile, scene-swap, first-render, and Three.js resources. A module planner worker retains the active song, uses project generations and request IDs, coalesces pending work, reports optional profiles, and leaves the active valid world usable while compiling. The client rejects stale results, invalidates on raw input, checks planned versus desired mix, and delays activation through a short quiet period. A 100-request unit burst activates only the newest result. In the dev browser, 10/80/10 compiles off-thread in 2.25 s with 2.38 s round-trip while frame p95 remains 16.8 ms and timeline stays at 3.500 s; scene replacement is 9.8 ms and first render still costs 119 ms. Target placement still evaluates 14,451 candidates and 8.5 million clearance samples. | Begin P2 in [Marble live-control performance](implementation/marble-live-control-performance.md): prune target candidates and add route-clearance broad-phase using P0 counters, then proceed to persistent scene/resource work in P3 |
