...
| Spectral Bloom SB0-SB5 | Geometry-first vertical slice implemented | engineering-preview | The analyzer now emits a versioned 24-band, 50 Hz master-audio field with phase sign, flux, centroid, spread, and flatness. `@reaper-viz/compiler-spectral-bloom` drives 16 fixed-step damped scalar/tangential modes with bounded deterministic coefficients. `@reaper-viz/scene-spectral-bloom` reconstructs one stable 31,000-particle surface/interior body on the GPU, supports absolute-time scrubbing and a slow observational camera, and is selectable in the preview. Compiler, topology, analyzer, full TypeScript, desktop, and mobile layout checks pass. The result establishes sound-shaped coherent geometry without literal bubble rendering, but remains a monochrome engineering preview. | Continue [SB6-SB9](implementation/spectral-bloom-implementation.md): transient detach/return, anticipation, unified fold/core material, stronger visual-classification fixtures, adaptive tiers, and two-song Q5 acceptance |
