...
| Physics-first shader worlds | Planning complete | planned | Five implementation-ready one-track concepts now share exact deadline ownership, offline deterministic compilation, absolute-time playback, future-target certification, seek-safe shader uniforms, camera visibility gates, and adaptive GPU budgets. Separate plans cover Phaseglass refraction, Singularity Slalom patched-conic gravity, Vortex Loom incompressible advection, Pendulum Cathedral constraints and ballistic transfers, and Aurora Cyclotron closed-form Lorentz helices. | Begin the recommended Aurora Cyclotron A0-A1 feasibility spike from the [shader worlds roadmap](implementation/shader-worlds-roadmap.md), then compare its reference-song diagnostics against Phaseglass before scene polish |
| Music-synchronized ecosystem worlds | Planning complete | future platform | A simulation-first roadmap defines deterministic agents, resources, spatial graphs, lifecycle processes, affordance extraction, up-to-four prominent musical voices, continuous track modulation, physics back-solving, shared space-time reservations, checkpoints, scalable populations, and causal music effects. Five example worlds are specified: Pulse District, Tidal Reef, Mycelial Canopy, Emberdeep Dungeon, and Halo Habitat. Music may schedule valid behavior or amplify a compatible world effect, but may not replace ecological logic with random beat motion. | When this research track begins, implement only the Pulse District E0 headless causal-loop fixture from the [ecosystem worlds roadmap](implementation/ecosystem-worlds-roadmap.md); do not build a universal framework first |
