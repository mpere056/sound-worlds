...
| Music-Synced Brick Breaker | In progress | selectable B1 preview | B0 deterministically selects one note-bearing track, groups chords, emits stable destruction deadlines, preserves the exact final note, and reports brick/cell counts plus gap diagnostics. The reference song requires 19 bricks with a 0.174479-second minimum gap. B1 has reflection and continuous collision primitives plus an absolute-time direct-contact performance and Pixi scene. `Brick Breaker · B1 Preview` is selectable for compiled projects, renders live bricks/compound cells, fragments, trail, paddle, and exact note-time removals, and explicitly reports that wall/paddle search is pending. | Replace preview grid contacts with bounded direct reflection candidates, then add B2 live-brick occupancy certification |
