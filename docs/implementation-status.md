...
| Marble Music M0-M1.5 | In progress | engineering-preview | `@reaper-viz/compiler-marble`, `@reaper-viz/scene-marble`, `compile:marble`, Three.js app routing, one-track auto-selection, compiled impacts/path/tail, camera keys, and compiler tests are implemented. Compiler v10 uses controlled horizontal/depth launch velocity and gravity to place note-time collisions, keeps pitch out of route geometry, and separates `target.contactPos` from rendered platform `target.pos`. Route planning now follows a five-forward/one-back depth cadence with a widened bounded corridor; platforms receive real depth tilt from three-dimensional collision normals. Marble clearance is sphere-to-oriented-box distance, while platform overlap uses the full 3D separating-axis set. Target candidates are ranked against a 120 Hz sample of the full route. `untitled-project-418cb58f` retains 19 exact impacts, spans depth 0.75-11.196, keeps average segment speed bounded at 1.483-2.023 with a 1.364 ratio, and has zero physical target intersections with 0.00738 minimum full-route clearance. The elevated camera smoothly follows 72% of marble depth in position and 88% in aim, keeping the marble visible without abrupt zoom. Measured projected movement is 19.8% lateral / 20.4% vertical / 59.9% depth, with 15 forward and 3 backward transitions. Tests require at least 52% visible depth contribution and cap lateral contribution at 30%. Unit and browser checks show continuous framing, clear approach/recede parallax, and no console warnings, but full audio watch-through and full-timeline viewport gates remain open. | Continue [Marble Music 3D physics-feel](implementation/marble-music-3d-physics-implementation.md): add full-timeline viewport projection coverage, run a full audio watch-through with exact-impact scrubs, then improve dense mechanism identity before cinematic camera offsets or two-track work |
