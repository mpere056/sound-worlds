...
| Spectral Bloom | Concept and implementation plan documented; implementation not started | reference-directed plan | One persistent 3D particle body is continuously deformed by master-audio analysis rather than note events. The approved plan interprets the cymatics reference as resonant geometry, not a literal bubble: signed spectral controls excite damped scalar and tangential spherical-harmonic modes, producing stable nodal lobes, folds, interference, and bounded transient detachment in one material. | Begin [SB0 contracts and fixtures](implementation/spectral-bloom-implementation.md) only after an explicit implementation request; preserve the monochrome geometry-first gate before particle polish |
