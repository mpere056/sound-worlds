...
| Marble live-control performance | In progress | engineering-preview | P0-P2 are complete; P3 persistent renderer ownership and primitive pooling are implemented. `benchmark:marble-live` reports deterministic compiler phases/work, while `?profileMarble=1` records frame, worker compile, scene application, first render, stable renderer identity, update count, and Three.js resources. The latest-only planner worker leaves the active valid world usable and rejects stale generations. P2 reduces extreme 10/80/10 from a 1,938 ms median to 105.8 ms p95. `MarbleScene.replacePerformance()` retains its renderer, context, scene, camera, lights, wall, marble, tuning, and one SVG overlay. Unit primitives and scene-lifetime static materials are shared across target bases, hardware, rods, ties, and supports. Browser geometry memory fell from 54-118 to 11; extreme application measured 2.2 ms with a 14.6 ms first render, while a subsequent balanced route retained the renderer and 11 geometries, applied in 4.5 ms, and first-rendered in 5.8 ms. | Continue P3 in [Marble live-control performance](implementation/marble-live-control-performance.md): retain stable target groups and reuse route buffers to consistently reach <=4 ms and pass the 100-change bounded-resource gate before P4 morphing |
