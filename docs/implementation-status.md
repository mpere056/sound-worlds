...
| Physics-first visual worlds | Two active engineering previews; three strengthened plans; one shelved experiment | in progress | Aurora Cyclotron and Phaseglass remain active engineering previews. Pendulum Cathedral, Singularity Slalom, and Lumenfall retain implementation-ready plans with distinct mechanism, orbital, and static-world ballistic foundations. Vortex Loom's certified physics experiment remains in the repository for learning and reference, but its concept and renderer are shelved after repeated visual review found that the shuttle-and-weave grammar did not produce a compelling music visualizer. | Continue Aurora and Phaseglass independently. Begin any Lumenfall implementation with its static-world and five-impact lighting proof. Do not continue Vortex Loom or reuse its transported-fiber presentation |
| Vortex Loom V0-V6 | Shelved 2026-07-16 | visually rejected experiment | The deterministic compiler, certified annulus interactions, flow kernel, checkpoints, tests, and preview implementation remain available as engineering evidence. Multiple renderer revisions improved continuity and mapping, but the core shuttle-and-weave presentation continued to read as visually messy and musically unconvincing in human review. This is a concept stop, not an invitation for another cosmetic pass. | No further implementation. Preserve the code and [shelved plan](implementation/shader-world-vortex-loom.md) until an explicit future decision archives or removes it |
| Spectral Bloom SB0-SB5 | Direct 3D waveform preview implemented | engineering-preview | The earlier damped-mode response was removed. The analyzer now stores 128 signed master-waveform samples plus 24 signed spectral bands for every 20 ms frame. The compiler packages and linearly interpolates those measured frames without force, velocity, damping, or state. The GPU scene wraps waveform samples around particle longitude, spectrum across latitude, and delayed waveform copies through depth. Zero audio reconstructs the exact baseline body at every silent frame. Sine/square identity, silence reset, seeking, analyzer, compiler, topology, browser, and shader-warning gates pass. | Continue the corrected [direct-waveform plan](implementation/spectral-bloom-implementation.md): binary/high-resolution analysis, stereo/spatial channels, improved spherical parameterization, material polish, adaptive tiers, and two-song Q5 acceptance |
| Waveform Halo WH0-WH3 | Direct contour-history preview implemented | engineering-preview | A separate compiler phase-aligns direct waveform frames by cyclic rotation without changing measured values. The Three.js scene maps the current frame to a bright aperture perimeter and 1.4 seconds of measured history to 84 GPU ribbon contours in perspective depth. Current activity opens both spread and depth; exact silence hides every history contour and leaves one baseline circle. Focused compiler, geometry, silence, identity, determinism, app-build, browser, and shader-warning gates pass. | Continue the [Waveform Halo plan](implementation/waveform-halo-implementation.md): production waveform resolution, binary artifacts, material hierarchy, stereo/spatial mapping, profiling, and two-song Q5 acceptance |
| Lumenfall L0-L8 | L0-L1 foundation and preliminary L2-L5 slice implemented | engineering-preview | `@reaper-viz/compiler-lumenfall` now groups one-note deadlines, builds a frozen 192-slab Nocturne Causeway graybox, solves analytic gravity arcs, emits passive reflection and musical-impulse diagnostics, and samples by absolute time. The 19-note reference has 19 exact contacts, roughly `1.4e-15` endpoint error, zero contact-plane penetrations, sub-`6.1 m/s` speed, and effectively zero tangential impulse ratio. `@reaper-viz/scene-lumenfall` renders generated basalt textures, wet/dry PBR materials, dark water, inverse-square body light, a coupled shadow spotlight, analytic impact light, velocity streak, trail, and following camera. App selection and timestamp query previews are wired. | Continue the [implementation plan](implementation/lumenfall-implementation.md): replace the continuous-forward heuristic with pair-state varied-normal search, add BVH swept-sphere certification against final terrain, improve comet silhouette and causeway assets, then pass bloom-off lighting, camera, performance, and full-song gates |
| Aurora Cyclotron A0-A4 | A0-A1 complete; A2-A4 in progress | exact physics driving an abstract volumetric field | `@reaper-viz/compiler-aurora` emits plan and performance artifacts. Its Lorentz kernel covers constant and divergence-free paraxial finite fields, fringe-region Boris integration, and work-energy diagnostics. The inverse route compiler scores planar, depth-oriented, and inward helical candidates while keeping physical coil axes distinct from arrival directions. A curvature window rejects legal-but-straight magnetic spans, and final-tail exit clearance closes the last unowned edge case. Oriented-torus certification feeds candidate scoring and checks coil bodies plus particle sweeps with conservative between-sample bounds. The reference song produces 19 exact coil-center crossings, a `4.23` maximum field under the `8.0` bound, `0.943` minimum center spacing, `0.392` minimum coil-surface clearance, `0.040` minimum particle clearance, a `19.665` maximum route radius, zero occupancy violations, and zero crossing error. A4 no longer visualizes those objects literally. Its half-resolution volume shader turns the particle into a folded singularity knot, coils into invisible curl/caustic operators, and history into a filament wake, combining folded inversion turbulence, log-polar coordinates, interference ridges, front-to-back density integration, and a shader filmic resolve. There are no visible spheres, toruses, route lines, or trail beads. A deterministic musical-state sampler now maps register, pitch direction, velocity, recent-note density, succession, exact pulses, and silence into field scale, handedness, pressure, sharpness, luminance, wake density, and negative space while remaining seek-safe. Pitch and velocity glide across note boundaries, dense attacks accumulate sustained phrase pressure instead of repeatedly reconfiguring geometry, and operator handoffs decay at their temporal window edges. A three-second anticipation field reserves negative space for upcoming notes, exposes their pitch and velocity through fractured boundary flow, and continuously converts each aperture into its arrival bloom and afterglow. A permanent world-anchored reference weave now supplies parallax and visibly deforms around those apertures, while analytic note-force impulses accelerate continuous forward advection and add pitch-weighted lateral drift. Early-aperture ownership, finite-fringe recertification, final field composition, and acceptance remain. | Add explicit aperture-ownership certification and bounded finite-fringe shooting correction, then continue Q1-Q5 composition, palette, density hierarchy, and volumetric refinement |
| Phaseglass P0-P6 | Compiler and refractive field implemented | engineering-preview | `@reaper-viz/compiler-phaseglass` and `@reaper-viz/scene-phaseglass` compile deterministic optical state and render one continuous refractive field. Notes now write bounded, multidimensional aberrations into shared glass instead of spawning disconnected objects: register, pitch class, interval direction, velocity, duration, and local density influence lens depth, astigmatic axis, spherical aberration, chromatic dispersion, caustic concentration, and relaxation. GPU failure diagnostics report shader compile/link failures, WebGL context loss, and black-frame output. The current preview has useful chromatic refraction and spatial deformation, but water-ripple dominance, weak glass reference structure, and insufficiently distinct note identities remain visual acceptance risks. | Preserve the continuous optical field, reduce ripple-like signatures, strengthen persistent refractive reference structure and true volume cues, then run Q3-Q5 note-identity and full-song acceptance |
