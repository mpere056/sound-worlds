...
| Marble Music M0-M1.5 | In progress | engineering-preview | `@reaper-viz/compiler-marble`, `@reaper-viz/scene-marble`, `compile:marble`, Three.js app routing, Pixi/Three backend ownership, one-track auto-selection, compiled note impacts/path/tail, camera keys, and compiler tests are implemented. Compiler v4 uses the MusicMarbles placement principle: every impact launches at a controlled horizontal speed under one gravity constant, and the next platform is placed at the predicted note-time collision position. Pitch now affects target identity rather than route geometry. On `untitled-project-418cb58f`, average segment speed is bounded to 1.64-2.76 world units/s instead of 1.59-8.77, with remaining acceleration caused by gravity; all 19 impacts remain exact. Platforms rotate from collision impulse, airborne motion is unrailed, contact normals follow platform surfaces, and spin accumulates. The camera now follows the marble pose directly in x/y and uses compiled keys for depth/zoom, keeping the initial marble clearly visible near center instead of framing the whole board. Browser review shows a visible initial marble and no console warnings, but the full human audio watch-through and automated full-timeline viewport gate remain open. | Continue [Marble Music 3D physics-feel](implementation/marble-music-3d-physics-implementation.md): add full-timeline viewport projection coverage, run a full audio watch-through, scrub exact impacts and between-note positions, improve dense/local mechanism readability, and only then plan cinematic camera offsets or two-track work |
