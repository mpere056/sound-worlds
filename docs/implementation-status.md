...
| Physics-first visual worlds | Two engineering previews; three strengthened plans | in progress | Aurora Cyclotron and Phaseglass now have implemented one-track engineering previews. Vortex Loom, Pendulum Cathedral, and Singularity Slalom have implementation-ready plans with deliberately different foundations: coherent field transport, readable nonlinear mechanics, and compiled orbital geometry with physically owned lensing. Vortex Loom now has a checkpoint-first V0-V10 execution plan covering an independent annulus-ownership oracle, deterministic beam and multiple-shooting solve, Lagrangian fiber transport, replayable material checkpoints, bounded musical mappings, diagnostics, fixtures, and Q0-Q5 promotion. All five share exact event ownership, deterministic offline compilation, anticipation, seek-safe state, certification artifacts, and the architecture-neutral Q0-Q5 [visual-quality standard](implementation/shader-worlds-visual-quality-standard.md). | Continue Aurora and Phaseglass visual acceptance independently. For the next unbuilt world, begin Pendulum Cathedral C0-C2 with a neutral-material mechanism proof before adding cathedral art direction or effects |
