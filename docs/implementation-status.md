...
| Aurora Cyclotron A0-A4 | A0-A1 complete; A2 and A4 graybox in progress | testable exact-route physics preview | `@reaper-viz/compiler-aurora` now emits both plan and performance artifacts. Its Lorentz kernel covers constant and divergence-free paraxial finite fields, fringe-region Boris integration, and work-energy diagnostics. The inverse route compiler deterministically scores planar, depth-oriented, and inward helical candidates while keeping physical coil axes distinct from arrival directions. The reference song produces 19 exact coil-center crossings, a `6.56` maximum field under the `8.0` bound, `0.899` minimum adjacent spacing, and zero compiled crossing error. `@reaper-viz/scene-aurora` is selectable in the app and provides a seek-safe Three.js physics graybox with solved toroidal coils, a followed particle, sampled trail, active-coil hierarchy, and a polar-night aurora fragment shader. Twenty-seven focused Aurora tests pass. Global occupancy, finite-fringe shooting correction, shader elevation, and acceptance remain. | Add global future-coil/rim collision certification and bounded fringe shooting correction before elevating the shader and hardware materials beyond Q0 |
