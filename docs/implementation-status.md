...
| Live Marble Music | Planned | design-ready | This is a separate runtime with webcam gestures, REAPER MIDI telemetry, fixed-step physics, and a rolling 5-8-platform certainty window. Future platforms remain lightweight and non-collidable in an uncertainty vortex, settle continuously into certified poses, and lock before activation. | Build telemetry/latency spike, uncertainty renderer, rolling solver, and certainty state machine |
