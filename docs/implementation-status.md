...
| Marble live-control performance | Planned | planned | Dedicated plan covers benchmark instrumentation, latest-only worker planning, warm-start/incremental solver work, persistent Three.js renderer and resource pools, collision-aware future-platform morphing, a high-rate input coordinator, worker-based MediaPipe Hand Landmarker integration, and a measured C++/Rust WASM decision gate | Start P0 in [Marble live-control performance](implementation/marble-live-control-performance.md), commit/push its benchmark baseline, then proceed to the planner worker before changing scene motion |
