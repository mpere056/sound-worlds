...
| Marble Music M0-M1.5 | In progress | engineering-preview | `@reaper-viz/compiler-marble`, `@reaper-viz/scene-marble`, `compile:marble`, Three.js app routing, Pixi/Three backend ownership, one-track auto-selection, compiled note impacts/path/tail, authored camera keys, and compiler tests are implemented. `untitled-project-418cb58f` proves the first mostly-successful one-track sync behavior: one marble, exact note impacts, sustain-aware holds, and back-solved travel into the next onset. M1.5 has begun with deterministic physical pose metadata: 3D tangents/normals, arc-length samples, distance-based spin, normalized quaternions, exact-impact pose tests, paired Three.js tube rails with ties/standoffs/collars, contact shadows, damped target recoil, wall-frame geometry, target hardware, pose-driven camera parallax, hidden SVG fallback, removed target-order line, softer rails/glow, and wider calmer default camera framing. Browser still-frame review on `untitled-project-418cb58f` now shows the graph-like overlay removed with no console warnings, but the full audio watch-through gate remains open. `untitled-project-6d2e04f7` compiles to 20 low-track impacts with zero dropped notes and audible-tail resonance | Continue [Marble Music 3D physics-feel](implementation/marble-music-3d-physics-implementation.md): run a full audio watch-through, scrub exact impacts/between-note holds, improve dense/local mechanism readability, do dense high-track manual stress review, and only then plan two-track duet work |
