...
| Marble Music M0-M1.5 | In progress | engineering-preview | `@reaper-viz/compiler-marble`, `@reaper-viz/scene-marble`, `compile:marble`, Three.js app routing, one-track auto-selection, compiled impacts/path/tail, camera keys, and compiler tests are implemented. Compiler v9 uses controlled horizontal/depth launch velocity and gravity to place note-time collisions, keeps pitch out of route geometry, and separates `target.contactPos` from rendered platform `target.pos`. Contact points weave through a bounded front/back volume and platforms receive real depth tilt from three-dimensional collision normals. Marble clearance is sphere-to-oriented-box distance, while platform overlap uses the full 3D separating-axis set. Target candidates are ranked against a 120 Hz sample of the full route and now prefer inward depth travel over stalling at a depth boundary. `untitled-project-418cb58f` retains 19 exact impacts, spans depth 0.25-4.25, keeps average segment speed bounded at 1.32-2.46 with a 1.87 ratio, and has zero physical target intersections with 0.0165 minimum full-route clearance. The camera uses a steeper elevated stable-depth perspective. Measured projected movement changed from 67% lateral / 26% vertical / 7% depth to 46% lateral / 25% vertical / 29% depth. Tests require at least 22% visible depth contribution and cap lateral contribution at 58%. Unit and browser checks show continuous framing, strong approach/recede scale changes, and no console warnings, but full audio watch-through and full-timeline viewport gates remain open. | Continue [Marble Music 3D physics-feel](implementation/marble-music-3d-physics-implementation.md): add full-timeline viewport projection coverage, run a full audio watch-through with exact-impact scrubs, then improve dense mechanism identity before cinematic camera offsets or two-track work |
