...
| Marble live-control performance | In progress | engineering-preview | P0-P3 are complete; P4 transform-only transitions and the first P5 high-rate coordinator slices are implemented. P2 reduces extreme 10/80/10 from a 1,938 ms median to roughly 100 ms p95. P3 retains one pooled renderer/scene. P4 holds transport and marble time, aligns the incoming route at that timestamp, and moves one fully visible platform set over a displacement-aware 450-1400 ms window before installing validated physics and resuming playback. Position, shortest-path rotation, collision-body size, colored carrier, hardware group, support rods, and stable-ID route rails move continuously; rails use pooled cylinder segments with moving ties/collars/supports instead of rebuilt tube geometry. No crossfade, reveal, ghost, duplicate target set, or transition opacity remains. Repeated worker plans retarget from displayed target, carrier, and path transforms, so continuous slider input does not restart from stale positions. Tiny collision pads remain on bounded marble-relative carriers, and scrub is briefly locked during re-layout so backward playback starts from a complete settled map. P5 exposes desired/requested/planned/active diagnostics and owns pure bounded 100% projection plus gesture-ready deadband/slew filtering; a five-minute 60 Hz simulation proves bounded 10 Hz planning cadence. P4 has a prepared-transition routing worker: it freezes exact displayed transforms, searches deterministic 3D staging arcs off-thread, requires a zero-overlap 120-sample certificate before movement, and terminates stale workers. Staged 24-sample candidate ranking, conflict-local repair, optional temporal staggering, and a transition-only 0.002 contact tolerance reduce the previously difficult 60/20/20 route from 1,890.7 ms to 78.3 ms while preserving final compiler clearance. Browser frame p95 is 17 ms and max 43.6 ms. P6 now includes the palm-normalized landmark controller plus an explicit webcam lifecycle and dedicated MediaPipe Hand Landmarker worker. It serves the model locally, transfers at most one 640x480 frame at a time, drops busy frames, keeps camera data local, stops tracks on teardown, and routes landmarks through the bounded live coordinator. Ignored permission prompts recover after 12 seconds without disabling slider control. | Validate successful webcam inference and sustained-session latency on the target machine, then tune gesture gain, thresholds, low-light behavior, and optional calibration against measured input |
...
| Music-Synced Brick Breaker | Planned for later | planned | Future deterministic one-ball generator is documented: one brick per distinct grouped note time, compound chord bricks, exact note-time destruction, legal direct/wall/paddle collision itineraries, time-varying live-brick occupancy, swept collision validation, bounded paddle motion, and a final-note final brick | Do not implement until offline one-track Marble Music passes its human gate; then begin B0 grouping/count diagnostics in [Music-Synced Brick Breaker](implementation/brick-breaker-implementation.md) |
