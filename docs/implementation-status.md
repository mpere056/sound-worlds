...
| Marble live-control performance | In progress | engineering-preview | P0-P2 are complete; P3 persistent renderer ownership and primitive pooling are implemented. `benchmark:marble-live` reports deterministic compiler phases/work, while `?profileMarble=1` records frame, worker compile, scene application, first render, stable renderer identity, update count, and Three.js resources. The latest-only planner worker leaves the active valid world usable and rejects stale generations. P2 reduces extreme 10/80/10 from a 1,938 ms median to 105.8 ms p95. `MarbleScene.replacePerformance()` retains its renderer, context, scene, camera, lights, wall, marble, tuning, and one SVG overlay. Unit primitives and scene-lifetime static materials are shared across target bases, hardware, rods, ties, and supports. Geometry memory fell from 54-118 to 11. A 100-update browser run retained renderer identity `1`, one overlay, 7 programs, and 11 geometries; final application was 1.3 ms, render p95 12.7 ms, and frame p95 17.1 ms. | Finish P3 in [Marble live-control performance](implementation/marble-live-control-performance.md): use the enhanced application/resource p95 diagnostics, then retain stable target groups and reuse route buffers if needed before P4 morphing |
