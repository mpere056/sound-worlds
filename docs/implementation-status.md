...
| Music-Synced Brick Breaker | In progress | collision-certified preview | B0 groups the selected note track into deterministic musical deadlines. B1 compiles a constant-speed, absolute-time arcade trajectory with brick-face reflection, wall bounces, paddle returns, trajectory-authored poses, and exact face clearance. B2 now uses bounded deterministic beam search plus oriented-box overlap and swept-circle occupancy checks: the ball cannot pass through or strike a live brick before its assigned beat. When no collision-safe intermediate brick exists, that deadline remains support/free travel; the final deadline must still break the final brick. The reference song certifies 18 bricks across 19 deadlines, skips one unsafe brick, and breaks the final brick at 9.916146 seconds. | Add route-search diagnostics and deliberate beat-aligned wall/paddle candidate templates, then broaden randomized occupancy fixtures |
