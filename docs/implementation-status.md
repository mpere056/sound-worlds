...
| Marble Music M0-M1.5 | In progress | engineering-preview | `@reaper-viz/compiler-marble`, `@reaper-viz/scene-marble`, `compile:marble`, Three.js app routing, one-track auto-selection, compiled impacts/path/tail, camera keys, and compiler tests are implemented. Compiler v6 uses controlled launch speed and gravity to place note-time collisions, keeps pitch out of route geometry, and separates `target.contactPos` from rendered platform `target.pos`. Platforms sit behind contact by marble radius plus target half-thickness and clearance. Target candidates are now ranked against a 120 Hz sample of the full route, so avoidable between-impact clipping is rejected or replaced by a shorter/rotated target; the 5.233 s reported fixture pose has 0.074 route clearance and the full fixture bottoms out at the intended 0.018 contact clearance. Launch direction prefers unoccupied route space, target sizes shrink or become compact pegs/chimes when crowded, and oriented footprints are compile-time validated. `untitled-project-418cb58f` retains 19 exact impacts and bounded 1.64-2.76 average speed with zero target-footprint intersections. The camera now follows a weighted time window around the marble and holds a stable relative distance/zoom instead of reacting to discontinuous bounce tangents or authored zoom changes. Unit and browser checks show continuous framing through impacts and no console warnings, but full audio watch-through and full-timeline viewport gates remain open. | Continue [Marble Music 3D physics-feel](implementation/marble-music-3d-physics-implementation.md): add full-timeline viewport projection coverage, run a full audio watch-through with exact-impact scrubs, then improve dense mechanism identity before cinematic camera offsets or two-track work |
