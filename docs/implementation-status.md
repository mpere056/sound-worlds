...
| Marble live-control performance | In progress | engineering-preview | P0-P3 are complete; P4 activation alignment, deferred playback, camera blending, and collision-checked future-target morphing are implemented. P2 reduces extreme 10/80/10 from a 1,938 ms median to 105.8 ms p95. P3's pooled renderer/scene passed 100 updates at 1.8 ms application p95 and 17.3 ms frame p95 with 11 geometries, 7 programs, renderer identity `1`, and one overlay. P4 rigidly aligns incoming route/targets at the next shared impact, waits for transport, and blends the camera over 350 ms. Same-shape targets after the boundary note morph during that window only when 12 sampled mixed layouts pass 3D SAT and active-marble sphere/OBB clearance; the current and played platforms stay fixed. The reference accepted 18 future targets for 20/21/59, showed a visible intermediate layout at 0.350 s with zero swaps, and activated once at 0.531 s on the same renderer with 11 geometries and 16.8 ms frame p95. Nine scene tests cover transition interpolation and collision rejection in addition to activation continuity. | Finish P4 in [Marble live-control performance](implementation/marble-live-control-performance.md): retain safe played-target transforms, run a naturally advancing audio watch-through, and close the remaining visual transition gates before the P5 high-rate coordinator |
