    ...
    configureServer(server) {
      server.middlewares.use(async (request, response, next) => {
        try {
          ...
          const match = /^\/api\/projects\/([^/]+)\/(song\.json|performance\.(runner|metro|painting|marble|brick-breaker|aurora|phaseglass|spectral-bloom|waveform-halo|vortex-loom)\.json|master\.wav)$/.exec(url.pathname);
