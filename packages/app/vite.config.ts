    ...
    configureServer(server) {
      server.middlewares.use(async (request, response, next) => {
        try {
          ...
          if (url.pathname === "/api/projects") {
            const projects = await Promise.all((await projectNames()).map(async (id) => {
              ...
              const concepts = [
                ...
                ...(existsSync(join(PROJECTS, id, "performance.waveform-halo.json")) ? ["waveform-halo"] : []),
                ...(existsSync(join(PROJECTS, id, "performance.lumenfall.json")) ? ["lumenfall"] : []),
          ...
          const match = /^\/api\/projects\/([^/]+)\/(song\.json|performance\.(runner|metro|painting|marble|brick-breaker|aurora|phaseglass|spectral-bloom|waveform-halo|lumenfall|vortex-loom)\.json|master\.wav)$/.exec(url.pathname);
